...
void Raster::writeToPPM(){
    ...
    for (int i = height - 1; i >= 0; i--){
        for (int j = 0; j < width; j ++){
            Color color = getColorPixel(j,i);
            if(j == width -1){
                output << round(color.red * 255) << " " << round(color.green * 255) << " " << round(color.blue * 255) << std::endl;
            }
            else{
                output << round(color.red * 255) << " " << round(color.green * 255) << " " << round(color.blue * 255) << " ";
            }
            count++;
        }
    }
